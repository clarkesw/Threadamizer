...
public class FunctionAndThenTest {
	...
	public static void main(String[] args) {
		...
		Function<String, Customer> returnCustomer = (String s) -> getCustomer(s);

		for (String customerDataFromCSV : customerDataCSV) {
			customer.add(returnCustomer.apply(customerDataFromCSV));
		}
		...
		// Use andThen since returnCustomer is already there.
		for (String customerDataFromCSV : customerDataCSV) {
			firstNameEmailHyphenSeparated.add(returnCustomer.andThen(hyphenSeparator).apply(customerDataFromCSV));
		}
